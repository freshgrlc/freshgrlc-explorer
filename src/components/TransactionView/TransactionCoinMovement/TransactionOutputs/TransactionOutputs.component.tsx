...
export const TransactionOutputs: React.FC<IProps> = ({ outputs }) => {
  ...
  const getDestinationHeader = (output: ITransactionOutput): string =>
    convertDestination(output)[0];
  const getDestinationRaw = (output: ITransactionOutput): string =>
    convertDestination(output)[1];
        ...
        <div key={"output" + index} className={classes.transactionOutput}>
          <div className={classes.destination}>
            <div
              className={
                classes.address +
                (output.type === "raw" ? " " + classes.rawOutput : "")
              }
            >
